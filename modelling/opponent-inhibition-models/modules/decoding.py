...
class LinearSVM(Decoder):
    ...
    def get_predicted_labels_train_test(self, training_set, test_set):
        clf = LinearSVC(dual = self.dual, C = self.C_best)
        clf.fit(self.responses_all.T[training_set,:], self.labels_all[training_set])
        self.predicted_labels = clf.predict(self.responses_all.T[test_set,:])
        return self.predicted_labels
